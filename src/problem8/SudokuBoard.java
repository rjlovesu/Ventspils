...
public class SudokuBoard
{
   ...
   * @return
   */
  public static SudokuBoard newEmptyBoard()
  {
   ...
   * @return
   */
  public static SudokuBoard newCopy(SudokuBoard board)
  {
  ...
  {
    SudokuBoard board = newEmptyBoard();
    ...
    for(String line : lines )
    {
      ...
      {
        board.setFieldValue(row, col, FieldValue.get( values[col].trim() ) );
      }
      row++;
    }
   ...
   * @return
   */
  public FieldValue setFieldValue(int row, int col, FieldValue value)
  {
    assert value != null;
   ...
   * @return
   */
  public FieldValue getFieldValue(int row, int col)
  {
    assert row >= 0 & row < SIZE;
    assert col >= 0 & col < SIZE;
   ...
   * @return
   */
  public FieldPosition getFreeFieldPosition()
  {
    for(int row = 0; row < SIZE; row ++)
    {
      for( int col = 0; col < SIZE; col++)
      {
        if( this.getFieldValue(row, col) == FieldValue.NONE )
        {
   ...
   * @return
   */
  public Set<FieldValue> getFieldValueCandidates(int row, int col)
  {
    assert row >= 0 & row < SIZE;
    assert col >= 0 & col < SIZE;
  ...
  public void pack()
  {
    ...
    for(int row = 0; row < SIZE; row ++)
    {
      for( int col = 0; col < SIZE; col++)
      {
        if( this.getFieldValue(row,col) == FieldValue.NONE )
        {
          Set<FieldValue> valueCandiadates = this.getFieldValueCandidates(row, col);
          if( valueCandiadates.size() ==  1)
          {
            for( FieldValue field : valueCandiadates )
            {
              this.setFieldValue(row, col, field );
            }
